...
wrapper.addEventListener("click", (e) => {
    ...
    if (gem && gem.tagName === "BUTTON" && state !== "ready") {
        ...
        let index;
        Object.entries(gemsCopy).map((g, i) => { if (g[1] === gem) {index = i} });

        const empty = document.querySelector(".empty");
        Object.entries(gemsCopy).map((g, i) => { if (g[1] === empty) {emptyIndex = i} });
...
function saveGameResults(time, moves, timerSeconds) {
  ...
  localStorage.setItem('gameResults', JSON.stringify(existingResults.slice(0, 5)));
}
